...
class _SearchListState extends State<SearchList> {
  ...
  List<DocumentSnapshot> documentsRequired = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: globalWidgets.myText(context, GlobalVariables.valueChosen == '' ? "Search List" : GlobalVariables.valueChosen, ColorsX.white, 0, 0,0,0, FontWeight.w400, 16),
        leading: IconButton(
  ...
  customItemDesign(BuildContext context){
    ...
    int length = documentsRequired.length ?? 0;
    return ListView.separated(
        itemCount: length,
        separatorBuilder: (context, index) =>Divider(height: 1, color: ColorsX.light_orange),
        shrinkWrap: true,
        ...
        itemBuilder: (context, index){
          return GestureDetector(
            onTap: (){
              ...
              GlobalVariables.idOfProposal = "${documentsRequired[index].reference.id}";
                ...
                children: <Widget>[
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      globalWidgets.myText(context, "${documentsRequired[index].get('caste')}", ColorsX.yellowColor, 5, 0, 0, 0, FontWeight.w900, 14),
                      ...
                      globalWidgets.myText(context, ageCalculate("${documentsRequired[index].get('dob')}") + " | " +heightCalculate("${documentsRequired[index].get('height')}"), ColorsX.white, 5, 0, 0, 0, FontWeight.w400, 12),
                      Container(
                        ...
                        child: globalWidgets.myTextCustom(context, "${documentsRequired[index].get('occupation')} | ${documentsRequired[index].get('qualification')}",
                            ColorsX.white, 5, 0, 0, 0, FontWeight.w400, 12),
                      ),
                      ...
                      Container(
                        ...
                        child: globalWidgets.myTextCustomOneLine(context, "${documentsRequired[index].get('city')} | ${documentsRequired[index].get('area')}", ColorsX.white, 5, 0, 0, 5, FontWeight.w600, 14),
                      ),
                      Visibility(
                        visible: "${documentsRequired[index].get('is_verified')}" == "1",
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      globalWidgets.myText(context, accountCreated("${documentsRequired[index].get('account_created_at')}"), ColorsX.white, 0, 0, 0, 0, FontWeight.w400, 14),
                      SizedBox(height: 3,),
                      // GestureDetector(
                      //     onTap: (){
                      //
  ...
  void loadProfessionData() async{
    ...
    if(firestoreResponseList.isEmpty) {
      ...
      setState(() {
        documentsRequired = querySnapshot.docs;
        ...
        print('professions length' + documentsRequired.length.toString());
      });
...
//     }
  ...
  void loadUserData() async{
    debugPrint(GlobalVariables.search_value.toString());
    GlobalWidgets.showProgressLoader("Please wait");

    if(GlobalVariables.search_by == "id"){

      final QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection('candidates')
          .where('id', isEqualTo: GlobalVariables.search_value.toString())
          .where('is_active_account', isEqualTo: '1')
          .limit(1)
          .get();
      final List<DocumentSnapshot> firestoreResponseList = querySnapshot.docs;
      if(firestoreResponseList.isEmpty) {
        ...
        setState(() {
          ...
          // GlobalVariables.featuredModelLength = documentsByCaste.length ?? 0;
          // GlobalVariables.featuredModelLength = featuredModel?.serverResponse.length ?? 0;
          print('castes length' + documentsByCaste.length.toString());
        });
        // print(documents.first);
        //
        // String id = querySnapshot.docs[0].reference.id;
    ...
    else if(GlobalVariables.search_by == "primary_phone"){

      final QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection('candidates')
          .where('primary_phone', isEqualTo: GlobalVariables.search_value)
          .where('is_active_account', isEqualTo: '1')
          .limit(1)
          .get();
      final List<DocumentSnapshot> firestoreResponseList = querySnapshot.docs;
      ...
      final QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection('candidates')
          .where('email', isEqualTo: GlobalVariables.search_value)
          .where('is_active_account', isEqualTo: '1')
          .limit(1)
          .get();
      final List<DocumentSnapshot> firestoreResponseList = querySnapshot.docs;
